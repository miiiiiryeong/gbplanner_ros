...
void Gbplanner::initializeAttributes() {
  ...
  // ADD traversability map subscriber : Miryeong
  traversability_map_sub = nh_.subscribe(
    "/travel_map", 1, &Gbplanner::traversabilityCallback, this);
}
...
// ADD traversabilityCallback : Miryeong
void Gbplanner::traversabilityCallback(const grid_map_msgs::GridMap& msg)
{
  try {
    grid_map::GridMapRosConverter::fromMessage(msg, traversability_map_);
    if (!traversability_map_.exists("traversability")) {
      ROS_WARN_THROTTLE(1.0, "Traversability layer not found in received grid map.");
      return;
    }
    has_traversability_map_ = true;
    ROS_DEBUG("Traversability map updated.");
  } catch (const std::exception& e) {
    ROS_ERROR("Failed to convert grid map message: %s", e.what());
  }
  std::cout << "Traversability map recieved!!" << std::endl;
}
